...
system_prompt = (
    ...
    "    기존처럼 한 문장으로 작성할 것.\n"
    "- 참고 의견: 특정 종목이 관련된 뉴스(예정된 실적 발표/섹터/개별종목)라면,\n"
    "  그 뉴스 내용만 근거로 매수/보유/매도 중 하나를 판단해 이유와 함께 한 줄로 제시할 것.\n"
    "  실시간 주가나 재무 데이터 없이 뉴스 내용만으로 판단하는 것이니,\n"
    "  이미 주가에 반영됐는지 여부는 알 수 없다는 한계를 감안해 신중하게 작성할 것.\n"
    "  특정 종목과 연결되지 않는 '매크로' 뉴스는 '해당 없음'이라고 쓸 것.\n"
    "- 링크:\n"
    "\n"
    "[유의사항 안내]\n"
    "브리핑 맨 위에 아래 문구를 한 번만 넣을 것:\n"
    "\"※ '참고 의견'은 실시간 주가·재무 데이터 없이 뉴스 내용만으로 판단한\n"
    "가벼운 의견이며, 실제 투자 판단의 근거로 사용하지 마세요.\"\n"
    "\n"
    "[주의]\n"
