...
- [x] 24단계: AI 요약에 '참고 의견'(매수/보유/매도 + 이유 한 줄) 항목 추가. 실시간 주가·재무 데이터 API 없이 뉴스 내용만으로 판단하는 가벼운 의견이라, 브리핑 맨 위에 "실제 투자 판단에 사용하지 말라"는 안내 문구를 한 번 넣도록 프롬프트 지시 추가 (`summarizer/summarize_news.py`). 특정 종목이 없는 매크로 뉴스는 '해당 없음'으로 처리. 기존 데이터로 테스트해 정상 작동 확인
- [ ] 25단계: (다음 단계 진행 시 여기에 기록)
